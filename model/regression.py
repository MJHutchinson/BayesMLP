...
class BayesMLPRegressionTester(Reg_NN):
    def __init__(self, input_size, hidden_size, output_size, training_size, y_mu, y_sigma,
                 no_train_samples=10, no_pred_samples=100, prev_means=None, prev_log_variances=None,
                 learning_rate=0.001,
                 prior_mean=0, prior_var=1, type = 'proper'):

        super(BayesMLPRegressionTester, self).__init__(input_size, hidden_size, output_size, training_size, y_mu, y_sigma)
        ...
        self.x2 = tf.placeholder(tf.float32, [None, None], name='x2')
        self.y2 = tf.placeholder(tf.float32, [None, None], name='y2')
        ...
        self.rmse = self._rmse(self.x2, self.y2)
        self.loglik = self._loglik(self.x2, self.y2)
        self.test_loglik = self._test_loglik(self.x2, self.y2)
        self.KL = tf.div(self._KL_term(), training_size)
