...
def mlp_definition(features,hidden):
  ...
  for lidx in range(0,len(hidden)):
    parent_layer = layers[-1]
    # Get number of hidden units
    num_units = hidden[lidx]
    if num_units==None: num_units=1
    # Define activation function
    # define next layer
    layers.append(tfp.layers.DenseLocalReparameterization(num_units,use_bias=True,activation=tf.nn.relu).apply(parent_layer))
